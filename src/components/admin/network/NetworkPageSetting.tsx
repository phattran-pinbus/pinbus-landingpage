...
import { networkSchema } from '../../../forms'
              ...
              {network &&
                network.map((network, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">{network.article_title}</td>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm w-72">
                      <figure className="relative pb-9/16">
                        {network.article_url_img && (
                          <Image
                            src={network.article_url_img}
                            alt={network.article_nanoid}
                            layout="fill"
                    ...
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                      <p className="text-gray-900 whitespace-no-wrap">
                        {moment(network.article_created_at).format('YYYY-MM-DD HH:mm')}
                      </p>
                    </td>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                      {remoteHTMLTags(network.article_content)}
                    </td>
                    ...
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                      <div className="flex">
                        <button
                          ...
                          onClick={() => setOpenEditItem(network.article_nanoid)}
                        >
                          <span
                            aria-hidden="true"
                        ...
                        <button
                          ...
                          onClick={() => setIsDeleting(network.article_nanoid)}
                        >
                          <span
                            aria-hidden="true"
...
const onCreate = async (formData: any) => {
    try {
      const {
        data: { message }
      } = await createArticle({...formData, article_section: 'network'})
      toast.success("Create successfully!")
      setOpenEditItem('')
      const newContent = [...network, message].sort(
        (a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
      )
      setNetwork(newContent)
    } catch (error) {
  ...
  const onSubmit = async (formData: any) => {
    try {
      ...
      const newContent = [...network.filter((i) => i.article_nanoid !== formData.article_nanoid), ...message].sort(
        (a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
      )
  ...
  const handleDelete = async () => {
    try {
      ...
      const newContent = network.filter((i) => i.article_nanoid !== isDeleting)
  ...
  return (
    <>
      <ModalFormComponent
        ...
        initialValue={!!openEditItem && openEditItem != 'create' ? network.find((i) => i.article_nanoid === openEditItem) : {}}
      ...
      <HeaderEditor sectionName="network" />
      <div className="relative bg-white p-6">
        <div className="items-center flex justify-between">
          <div className="text-xl font-inter">Network</div>
          <div>
...
export default NetworkPageSetting
