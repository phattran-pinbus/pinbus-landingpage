...
import { adsSchema } from '../../../forms'
import { toast } from 'react-toastify'

const BecomeAdsPostComponent = () => {
  ...
  const onSubmit = async (formData: any) => {
    try {
      ...
      const newContent = [...ads.filter((i) => i.article_nanoid !== formData.article_nanoid), ...message]
      setAds(newContent)
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      ...
      const newContent = ads.filter((i) => i.article_nanoid !== isDeleting)
  ...
  const onCreate = async (formData: any) => {
    try {
      ...
      setOpenEditItem('')
      const newContent = [...ads, message].sort(
  ...
  return (
    <>
      <ModalFormComponent
        ...
        schema={adsSchema}
        initialValue={
          !!openEditItem && openEditItem != 'create' ? ads.find((i) => i.article_nanoid === openEditItem) : {}
        }
      ...
      <div className="relative bg-white p-6">
        <div className="items-center flex mb-4 justify-between">
          <div className="text-xl font-inter">Advertisements</div>
          <div>
            <button
...
export default BecomeAdsPostComponent
